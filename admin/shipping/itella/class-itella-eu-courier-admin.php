...
class SWI_Itella_EU_Courier_Admin {

    public function __construct() {
        add_action( 'woocommerce_admin_order_data_after_shipping_address', [ $this, 'show_order_info' ] );
        add_action( 'wp_ajax_swi_iec_send',  [ $this, 'ajax_send' ] );
        add_action( 'wp_ajax_swi_iec_label', [ $this, 'ajax_label' ] );

        if ( get_option( 'swi_iec_auto_send' ) === 'yes' ) {
        ...
        if ( get_option( 'swi_iec_add_tracking' ) === 'yes' ) {
            add_action( 'woocommerce_email_order_details', [ $this, 'add_tracking_to_email' ], 10, 4 );
        }
    }

    private function order_uses_iec( WC_Order $order ): bool {
        foreach ( $order->get_shipping_methods() as $method ) {
            if ( strpos( $method->get_method_id(), 'swi_itella_eu_courier' ) !== false ) {
                return true;
            }
        }
        return false;
    ...
    private function do_send( WC_Order $order ) {
        ...
        if ( $order->get_meta( SWI_Itella_EU_Courier_Shipping::BARCODE_META ) ) {
            return new WP_Error( 'already_sent', 'Pakiandmed on juba saadetud.' );
        }

        $license_key = get_option( 'swi_iec_license_key', '' );
        if ( ! $license_key ) {
            return new WP_Error( 'no_license', 'Itella kuller EU litsentsi võti puudub seadistustes.' );
        }

        $time      = $order->get_meta( SWI_Itella_EU_Courier_Shipping::PICKUP_TIME_META );
        ...
        $payload = [
            ...
            'sender'      => [
                'name'  => get_option( 'swi_iec_sender_name',  '' ),
                'phone' => get_option( 'swi_iec_sender_phone', '' ),
                'email' => get_option( 'swi_iec_sender_email', '' ),
            ],
            'recipient'   => [
        ...
        $resp = wp_remote_post( $base_url . '/api/itella/eu/courier/shipment', [
            'timeout' => 15,
            'headers' => [
        ...
        if ( $code !== 200 || empty( $body['barcode'] ) ) {
            $msg = $body['error'] ?? ( 'Itella EU API viga (HTTP ' . $code . ')' );
            return new WP_Error( 'api_error', $msg );
        }

        $barcode = sanitize_text_field( $body['barcode'] );
        $order->update_meta_data( SWI_Itella_EU_Courier_Shipping::BARCODE_META, $barcode );
        $order->save();

        if ( get_option( 'swi_iec_send_label_copy' ) === 'yes' ) {
            $copy_email = get_option( 'swi_iec_label_email', '' );
            if ( $copy_email ) {
    ...
    private function send_label_email( int $order_id, string $barcode, string $to, string $license_key, string $base_url ): void {
        $size = get_option( 'swi_iec_label_size', 'A4' );
        $resp = wp_remote_get(
            $base_url . '/api/itella/eu/courier/label?barcode=' . urlencode( $barcode ) . '&order_id=' . $order_id . '&size=' . urlencode( $size ),
            [
                'timeout' => 30,
                'headers' => [
        ...
        $dest = sys_get_temp_dir() . '/pakisilt_iec_' . $order_id . '_' . uniqid() . '.pdf';
    ...
    public function maybe_auto_send( int $order_id ): void {
        ...
        if ( ! $order || ! $this->order_uses_iec( $order ) ) return;
        if ( $order->get_meta( SWI_Itella_EU_Courier_Shipping::BARCODE_META ) ) return;

        $result = $this->do_send( $order );
        if ( is_wp_error( $result ) ) {
            $order->add_order_note( 'Itella kuller EU — automaatne saatmine ebaõnnestus: ' . $result->get_error_message() );
        }
    }
    ...
    public function add_tracking_to_email( WC_Order $order, bool $sent_to_admin, bool $plain_text, object $email ): void {
        ...
        $barcode = $order->get_meta( SWI_Itella_EU_Courier_Shipping::BARCODE_META );
        if ( ! $barcode || ! $this->order_uses_iec( $order ) ) return;
        ...
        if ( $plain_text ) {
            ...
            echo '<p style="margin:16px 0;">Sinu pakk on teel — <strong>Itella kuller EU</strong>.<br>'
    ...
    public function show_order_info( WC_Order $order ): void {
        if ( ! $this->order_uses_iec( $order ) ) return;

        $time     = $order->get_meta( SWI_Itella_EU_Courier_Shipping::PICKUP_TIME_META );
        $barcode  = $order->get_meta( SWI_Itella_EU_Courier_Shipping::BARCODE_META );
        $times    = SWI_Itella_EU_Courier_Shipping::get_pickup_times();
        $nonce    = wp_create_nonce( 'swi_iec_nonce' );
        $order_id = $order->get_id();
        ?>
        <div style="margin-top:12px;padding:12px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;">
            <strong style="font-size:12px;color:#15803d;">🌍 Itella kuller EU</strong><br>
            <?php if ( $time ) : ?>
            ...
            <div style="margin-top:8px;">
                <a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=swi_iec_label&order_id=' . $order_id . '&nonce=' . $nonce ) ); ?>"
                   target="_blank" class="button button-small">📄 Prindi pakisilt</a>
            </div>
            <?php else : ?>
            <div style="margin-top:8px;">
                <button type="button" class="button button-small"
                        id="swi-iec-send-btn-<?php echo esc_attr( $order_id ); ?>"
                        onclick="swiIecSend(<?php echo (int) $order_id; ?>, '<?php echo esc_js( $nonce ); ?>')">
                    📤 Saada serverisse
                </button>
                <span id="swi-iec-send-result-<?php echo esc_attr( $order_id ); ?>" style="margin-left:8px;font-size:13px;"></span>
            </div>
            <script>
            function swiIecSend(orderId, nonce) {
                if ( ! confirm('Saata pakiandmed Itella EU serverisse?') ) return;
                var btn = document.getElementById('swi-iec-send-btn-' + orderId);
                var res = document.getElementById('swi-iec-send-result-' + orderId);
                btn.disabled = true;
                res.textContent = 'Saadan...';
                res.style.color = '#6b7280';
                jQuery.post(ajaxurl, {action: 'swi_iec_send', order_id: orderId, nonce: nonce}, function(r) {
                    btn.disabled = false;
                    if (r.success) {
    ...
    public function ajax_send(): void {
        check_ajax_referer( 'swi_iec_nonce', 'nonce' );
    ...
    public function ajax_label(): void {
        if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['nonce'] ?? '' ) ), 'swi_iec_nonce' ) ) {
            wp_die( 'Vigane nonce.' );
        }
        if ( ! current_user_can( 'manage_woocommerce' ) ) {
        ...
        $new_status = get_option( 'swi_iec_status_after_label', '' );
